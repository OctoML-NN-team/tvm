...
def _get_default_config(cfg, data, kernel, strides, padding, out_dtype, is_depthwise=False):
    batch_size, in_channel, ih, iw = get_const_tuple(data.shape)
...
def _pack_kernel(data, kernel, ic_bn, oc_bn):
    oc, ic, kh, kw = get_const_tuple(kernel.shape)
...
@autotvm.register_topi_compute("conv2d_NCHWc.IrisXe")
def conv2d_NCHWc(
    ...
    if in_channel % ic_bn != 0:
        ic_chunk += 1

    rc = te.reduce_axis((0, ic_chunk * ic_bn), name="rc")
    ry = te.reduce_axis((0, kernel_height), name="ry")
    rx = te.reduce_axis((0, kernel_width), name="rx")

    c_o = out_channel // oc_bn
    if out_channel % oc_bn != 0:
        c_o = out_channel // oc_bn + 1
    cshape = (batch, c_o, out_height, out_width, oc_bn)
...
@autotvm.register_topi_schedule("conv2d_NCHWc.IrisXe")
...
def _schedule_cl_spatialpack_NCHWc(cfg, s, op):
    ...
    temp_W = s.cache_read(temp, "shared", [output])
