...
def _get_model(
    ...
    w_s = weight_shape[0] * weight_shape[1] * weight_shape[2] * weight_shape[3]
    w_vals = [1.0 for i in range(w_s)]
    #w = tvm.nd.array(np.random.uniform(-128, 127, weight_shape).astype(dtype))
    w = tvm.nd.array(np.array(w_vals).reshape(weight_shape).astype(dtype))
...
def compare_inference_with_ref(func, params, atol=0.002, rtol=0.007):
    ...
    for free_param in analysis.free_vars(func):
        ...
        i_s = shape[0] * shape[1] * shape[2] * shape[3]
        i_vals = []
        for i in range(i_s):
            i_vals.append(i)
        inputs[name] = tvm.nd.array(np.array(i_vals).reshape(shape).astype(dtype))
