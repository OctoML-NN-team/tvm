    ...
    auto createTensor = [&](JSONGraphNodeEntry entry) {
      ...
      if (data != nullptr) {
        ...
        if (data_entry_[entry.id_]->device.device_type == kDLMetal)
          std::cout << counter << ". BIAO: metal: " << data_entry_[entry.id_]->device.device_type
                    << std::endl;
        else
          std::cout << counter
                    << ". BIAO: something else: " << data_entry_[entry.id_]->device.device_type
                    << std::endl;
        Device dev = {kDLMetal, 0};
        id<MTLDevice> device = mw->GetDevice(dev);
        id<MTLBuffer> buf =
            [device newBufferWithBytes:data
                                length:dlshape[0] * dlshape[1] * dlshape[2] * sizeof(dtype)
                               options:MTLResourceStorageModeShared];
        matrix_eid_[entry.id_] = [[MPSMatrix alloc] initWithBuffer:buf
                                                        descriptor:matrix_desc_eid_[entry.id_]];
        // id<MTLBuffer> buf = (id<MTLBuffer>)data;
        // matrix_eid_[entry.id_] = [[MPSMatrix alloc] initWithBuffer:buf
        // descriptor:matrix_desc_eid_[entry.id_]]; [buf release];
      }
    };
  ...
  void MatMul(const size_t& nid) {
    ...
    auto matmul = [aIdx, bIdx, dstIdx, this]() {
      ...
      [sgemm encodeToCommandBuffer:cb
                        leftMatrix:matrix_eid_[aIdx]
                       rightMatrix:matrix_eid_[bIdx]
                      resultMatrix:matrix_eid_[dstIdx]];
      //[mul_obj release];
