...
PackedFunc WrapTimeEvaluator(PackedFunc pf, Device dev, int number, int repeat, int min_repeat_ms,
                             PackedFunc f_preproc, TVMStreamHandle stream) {
  ICHECK(pf != nullptr);
  ...
  auto ftimer = [pf, dev, number, repeat, min_repeat_ms, f_preproc, stream](
    ...
    DeviceAPI::Get(dev)->StreamSync(dev, stream);
      ...
      if (opt_mod.defined()) {
        ...
        if (tkey == "rpc") {
          return static_cast<RPCModuleNode*>(m.operator->())
              ->GetTimeEvaluator(name, dev, number, repeat, min_repeat_ms, f_preproc_name, stream);
        } else {
        ...
        return WrapTimeEvaluator(*pf, dev, number, repeat, min_repeat_ms, f_preproc, stream);
      }
    });
